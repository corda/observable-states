...
class CampaignTests {
    ...
    fun `Start new campaign tests`() {
        ledgerServices.ledger {
            ...
            transaction {
                ...
                command(partyKeys(A), CampaignContract.Start())
                this.verifies()
            }
            ...
            transaction {
                output(CampaignContract.CONTRACT_REF, newValidCampaign)
                command(partyKeys(B, A), CampaignContract.Start())
                this.fails()
            }
    ...
    fun `Make a pledge tests`() {
        ...
        ledgerServices.ledger {
            ...
            transaction {
                ...
                output(PledgeContract.CONTRACT_REF, defaultPledge)
                command(partyKeys(A, B), PledgeContract.Create())
                command(partyKeys(B), CampaignContract.AcceptPledge())
                timeWindow(Instant.now(), 5.seconds)
                this.fails()
            }
            ...
            transaction {
                ...
                output(PledgeContract.CONTRACT_REF, defaultPledge)
                command(partyKeys(A, B), PledgeContract.Create())
                command(partyKeys(B, A), CampaignContract.AcceptPledge())
                timeWindow(Instant.now(), 5.seconds)
                this.fails()
            }
    ...
    fun `End campaign in success`() {
        ...
        ledgerServices.ledger {
            transaction {
                input(CampaignContract.CONTRACT_REF, endedCampaign)
                input(PledgeContract.CONTRACT_REF, Pledge(500.POUNDS, B.party, A.party, endedCampaign.linearId))
                input(PledgeContract.CONTRACT_REF, Pledge(200.POUNDS, D.party, A.party, endedCampaign.linearId))
                input(PledgeContract.CONTRACT_REF, Pledge(300.POUNDS, C.party, A.party, endedCampaign.linearId))
